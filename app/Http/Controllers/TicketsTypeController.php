...
class TicketsTypeController extends Controller
{
    ...
    {
        /*if (!Auth::user()->can('viewAny', TicketsType::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $filters    =   $request->input('filter')   ;
        $modelo = TicketsType::query();
        if(isset($filters))
        {
    ...
    {
        /*if (!Auth::user()->can('create', TicketsType::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $validator = Validator::make($request->all(), [
            'name' => 'required|string|unique:tickets_type',
    ...
    {
        $modelo = TicketsType::findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        /*
        $validator = Validator::make($request->all(), [
            'name' => ['required', 'string', Rule::unique('tickets_type', 'name')->ignore($id)],
            'description' => 'nullable|string'
        ]);
