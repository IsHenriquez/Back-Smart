...
class TicketsCategoryController extends Controller
{
    ...
    {
        /*if (!Auth::user()->can('viewAny', TicketsCategory::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $filters    =   $request->input('filter')   ;
        $modelo = TicketsCategory::query();
        if(isset($filters))
        {
    ...
    {
        /*if (!Auth::user()->can('create', TicketsCategory::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $validator = Validator::make($request->all(), [
            'name' => 'required|string|unique:tickets_category',
    ...
    {
        $modelo = TicketsCategory::findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        /*
        $validator = Validator::make($request->all(), [
            'name' => ['required', 'string', Rule::unique('tickets_category', 'name')->ignore($id)],
            'description' => 'nullable|string'
        ]);
        ...
        */
        ...
        $modelo = TicketsCategory::find($id);
