...
class TicketsPriorityController extends Controller
{
    ...
    {
        /*if (!Auth::user()->can('viewAny', TicketsPriority::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $filters    =   $request->input('filter')   ;
        $modelo = TicketsPriority::query();
        if(isset($filters))
        {
    ...
    {
        /*if (!Auth::user()->can('create', TicketsPriority::class)) {
            return response()->json(['error' => 'Sin autorización.', 'success' => false], 403);
        }*/

        $validator = Validator::make($request->all(), [
            'name' => 'required|string|unique:tickets_priority',
    ...
    {
        $modelo = TicketsPriority::findOrFail($id);
    ...
    public function update(Request $request, $id)
    {
        /*
        $validator = Validator::make($request->all(), [
            'name' => ['required', 'string', Rule::unique('tickets_priority', 'name')->ignore($id)],
            'description' => 'nullable|string'
        ]);
        ...
        */
        $modelo = TicketsPriority::find($id);
